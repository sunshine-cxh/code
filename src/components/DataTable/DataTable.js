...
class DataTable extends Component {
      ...
      if (props.record.children.length > 0) {
        if (props.expanded) {
          return (
            ...
            >
              <Icon antd type="down" style={{ fontSize: 14, marginRight: 4 }} />
            </a>
          )
        } else {
          return (
        ...
        return <span style={{ marginRight: 18 }}></span>
      }
    } else {
      return <span></span>
    }
  ...
  render() {
    ...
    const _rowSelection = {
      ...
      onSelect: (record, selected, selectedRows, event)=> {this.props.onSelectRow && this.props.onSelectRow(record, selected, selectedRows, event)},
      ...rowSelection,
    }
...
export const Tip = (prop) => (
  <Tooltip placement="top" title={prop.children}>
    <div className="ellipsis-2" style={prop.style}>
      {prop.children}
    </div>
  </Tooltip>
)
...
export const Paging = ({
  ...
  return (
    <div className={`${divWrap ? 'div-footer' : 'page-footer'}`}>
      <Pagination {...paging} />
    </div>
  )
}
