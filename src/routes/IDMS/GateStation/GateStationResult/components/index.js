 ...
 * @LastEditTime : 2020-08-14 14:20:31
 */
...
class GateStationResult extends BaseComponent {
  state = {
    choiceDate:[],
    disableDate:'',  //禁用选择的范围
    choosenDate:[],  //日期范围
  ...
  componentDidMount(){
    ...
    dispatch({
      type:'gateStationResult/getStationExitList'
    })
  }

  getChecked=(obj)=>{
    // let { paramsList } = this.props.gateStationResult
  ...
  handleSearch = ()=>{
    ...
    dispatch({
      type:'gateStationResult/getResult',
      payload:{
  ...
  checkedAllFn = (type, key, checked) =>{
    let {dispatch, gateStationResult:{ stationExitList }} = this.props
    let newArr = []
  ...
  render() {
    ...
    let {stationExitList,resultData} = this.props.gateStationResult
    let rangeProps = {
      type:'range',
      value: choosenDate,
      ...
      onOpenChange:(status)=>{
        ...
        if(!status && choosenDate.length<=1){
          this.setState({choiceDate:[],dateString:[],choosenDate:[]})
        }
      }
    }
...
export default GateStationResult
