/*
 * @Descripttion : 巡检路线新增页面
 * @Author       : caojiarong
 * @Date         : 2020-08-18 14:12:55
 * @LastEditors  : caojiarong
 * @LastEditTime : 2020-08-19 15:03:59
 */
  ...
  subscriptions: {
    setup({ history, dispatch }) {
      return history.listen(({ pathname }) => {
        if (pathname === `${routePrefix}/lineManage/subLineAdd`) {
          dispatch({
            type: '@change',
            payload: {
              basicInfos: {},
  ...
  effects: {
    ...
    *submit({ payload }, { call, put, select }) {
      ...
      yield put.resolve({
        type: '@request',
        payload: {
          url: `${apiPrefix}/inspectionroute/submit`,
          data: postData,
          success: () => {
            success()
          },
    ...
    *getSelectData({ payload }, {call, put, select }){
      yield put.resolve({
        ...
        payload:{
          url:`${apiPrefix}/inspectionpoint/getselectordata`,
          valueField: 'lineInfo'
        }
      })
    }
