...
export default modelEnhance({
    ...
    setup({ history, dispatch }) {
      return history.listen(({ pathname }) => {
        dispatch({
          type: '@change',
          payload: {
            isCurrentRoute: pathname == routePrefix+'/lineManage' ? true : false,
          }
        })
      });
    }
  ...
  effects: {
    ...
    *getPageData({ payload }, { call, put, select }){
      const { parameters } = yield select(state => state.lineManage)
    ...
    *getLineStatus({ payload }, { call, put, select }) {
      const { lineStatus } = yield select(state => state.lineManage)
