 ...
 * @LastEditTime : 2020-07-01 14:58:40
 */
...
export default modelEnhance({
  ...
  state: {
    ...
    functionExpandedKeys: [], //模块权限树形展开keys
    ...
    organizationPermissionTreeSelected: [], //已选组织机构权限
  },
  ...
  effects: {
    ...
    *getFunctionPermission({ payload }, { put, select }) {
      ...
      let functionExpandedKeys = array.map((item) => item.key)
    ...
    *submitFunctionPermission({ payload }, { put, select }) {
      // let { submitFunctionCheckedKeys } = yield select((state) => state.role)
