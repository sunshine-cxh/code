...
export default class extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (!isEqual(this.props.patrolTaskSubmit.details.photoList, prevProps.patrolTaskSubmit.details.photoList)) {
      this.setImageList()
    }
  }
  setImageList = () => {
  ...
  handleRemove = (file) => {
    let { dispatch, patrolTaskSubmit: { imageList } } = this.props
    let arr = imageList.filter(item => {
      return item.id !== file.id
    })
    dispatch({
      type: 'patrolTaskSubmit/deleteImage',
      payload: {
