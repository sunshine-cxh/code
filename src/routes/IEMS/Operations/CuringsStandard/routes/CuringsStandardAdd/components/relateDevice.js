 ...
 * @LastEditors  : hezihua
 * @LastEditTime : 2020-09-09 11:05:25
 */
...
export default class extends Component {
  ...
  deleteFn = (record) => {
    let {
      curingsStandardAdd: { appPageData, selectedRow, selectedRowKeys, addRow },
      dispatch,
    } = this.props
    if (!record.consume) {
      // 删除Keys 列表中的key
  ...
  componentWillUnmount () {
    let { curingsStandardAdd: { appPageData }, dispatch } = this.props
    appPageData.list = []
    dispatch({
      type: 'curingsStandardAdd/@change',
      payload: {
  ...
  // 手动添加
  handleSelectBtn = (e) => {
    let { dispatch,curingsStandardAdd: { selectDataList } } = this.props
    dispatch({
  ...
  render() {
    ...
    let { appPageData, editDeviceDataList } = curingsStandardAdd
