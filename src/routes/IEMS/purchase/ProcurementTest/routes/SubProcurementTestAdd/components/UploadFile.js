 ...
 * @LastEditors  : hezihua
 * @LastEditTime : 2020-09-04 14:17:52
 */
...
export default class extends Component {
  ...
  handleUpload = (e) => {
    ...
    if (e.file.response) {
      ...
      if (code === 0) {
        let {
          procurementTestAdd: { filePageData },
          dispatch,
        } = this.props
  ...
  deleteFn = (record) => {
    ...
    dispatch({
      type: 'procurementTestAdd/deleteFile',
      payload: {
        record,
      },
    })
  ...
  render() {
    let { procurementTestAdd } = this.props
    let { filePageData } = procurementTestAdd
    let { fileLoading } = this.state
    let columnsFile = createColumnsFile(this)
    let action = getAction(40)
