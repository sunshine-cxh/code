 ...
 * @LastEditors  : hezihua
 * @LastEditTime : 2020-09-04 14:23:21
 */
...
export default class extends Component {
  ...
  handleUpload = (e) => {
    if (e.file.response) {
      ...
      if (code === 0) {
        let {
          procurementPlanAdd: { filePageData },
          dispatch,
        } = this.props
  ...
  deleteFn = (record) => {
    ...
    dispatch({
      type: 'procurementPlanAdd/deleteFile',
      payload: {
        record,
      },
    })
  ...
  render() {
    let { procurementPlanAdd } = this.props
    let { filePageData } = procurementPlanAdd
