 ...
 * @LastEditors  : hezihua
 * @LastEditTime : 2020-06-19 09:29:57
 */
...
export default class extends Component {
  ...
  handleUpload = (e) => {
    ...
    if (e.file.response) {
      ...
      if (code === 0) {
        let {
          procurementApplyEquipment: { filePageData },
          dispatch,
        } = this.props
  ...
  deleteFn = (record) => {
    ...
    dispatch({
      type: 'procurementApplyEquipment/deleteFile',
      payload: {
        record,
      },
    })
  ...
  render() {
    let { procurementApplyEquipment } = this.props
    let { filePageData } = procurementApplyEquipment
