 ...
 * @LastEditors  : hezihua
 * @LastEditTime : 2020-09-04 14:23:09
 */
...
export default class extends Component {
  ...
  handleUpload = (e) => {
    ...
    if (e.file.response) {
      ...
      if (code === 0) {
        let {
          procurementApplyConsumables: { filePageData },
          dispatch,
        } = this.props
  ...
  deleteFn = (record) => {
    ...
    dispatch({
      type: 'procurementApplyConsumables/deleteFile',
      payload: {
        record,
      },
    })
  ...
  render() {
    let { procurementApplyConsumables } = this.props
    let { filePageData } = procurementApplyConsumables
    let { fileLoading } = this.state
    let columnsFile = createColumnsFile(this)
    let action = getAction(20)
