...
export default modelEnhance({
  ...
  effects: {
    ...
    *init({ payload }, { call, put, select }) {
      let { pageData } = yield select((state) => state.supplier)
      yield put.resolve({
        type: 'getPageInfo',
    ...
    *getPageInfo({ payload }, { call, put, select }) {
      const { parameters } = yield select((state) => state.supplier)
    ...
    *delete({ payload }, { call, put, select }) {
      ...
      yield put.resolve({
        ...
        payload: {
          notice: true,
          url: `${apiPrefix}/supplier/delete?id=${id}`,
        },
      })
