 ...
 * @LastEditors  : hezihua
 * @LastEditTime : 2020-09-09 10:37:35
 */
...
export default modelEnhance({
  ...
  subscriptions: {
    setup({ history, dispatch }) {
      return history.listen(({ pathname }) => {
        if (pathname === `${routesPrefix}/scrap/subScrapDetail`) {
          dispatch({
            type: '@change',
            payload: {
              details: {},
  ...
  effects: {
    *init({ payload }, { call, put, select }) {
      const { moveTableList, filePageData } = yield select((state) => state.scrapDetail)
      let { id } = payload
      if (id) {
        yield put.resolve({
          type: 'getDetails',
        ...
        const { details } = yield select((state) => state.scrapDetail)
