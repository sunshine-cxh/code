 ...
 * @LastEditTime : 2020-08-06 10:34:25
 */
...
class TodayGasChart extends BaseComponent {
  render() {
    let { simulation : { flowData }} = this.props
    //压力曲线图
    let flowDv = new DataSet.View().source(flowData)
    flowDv.transform({
      type: 'fold',
      fields: getFilterName(flowData, 'time'),
      key: 'type',
      value: 'value',
    })
    const flowScale = {
    ...
    return (
      <>
        <p className="today-gas-title">流量统计</p>
          ...
          >
            ...
            <Geom
              ...
              color={[
                'type',
                (type) => {
                  let typeList = getFilterName(flowData, 'time')
